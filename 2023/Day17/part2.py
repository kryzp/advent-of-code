...
def next_states(st):
	...
	if move_counter < 10:
		if is_valid_x(px + curr_dir_x) and is_valid_y(py + curr_dir_y):
			disthere = ll[py + curr_dir_y][px + curr_dir_x]
			yield disthere, (px + curr_dir_x, py + curr_dir_y, move_counter + 1, curr_dir_idx)
	...
	if move_counter >= 4:
		...
		if is_valid_x(px + dir1[0]) and is_valid_y(py + dir1[1]):
			...
			yield disthere, (px + dir1[0], py + dir1[1], 1, (curr_dir_idx + 1) % 4)

		if is_valid_x(px + dir2[0]) and is_valid_y(py + dir2[1]):
			disthere = ll[py + dir2[1]][px + dir2[0]]
			yield disthere, (px + dir2[0], py + dir2[1], 1, (curr_dir_idx + 3) % 4)
...
for start_state in [(0, 0, 0, 2), (0, 0, 0, 3)]:
	...
	while nodes:
		...
		if curr[0] >= len(ll[0]) - 1 and curr[1] >= len(ll) - 1 and curr[2] >= 4:
...
#				positions.append((cc[0], cc[1]))
		...
		for cost, next_st in next_states(curr):
			dist_st_next = dist_st_curr + cost
